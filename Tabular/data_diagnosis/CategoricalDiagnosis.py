import pandas as pd

from .Diagnosis import Diagnosis

class CategoricalDiagnosis(Diagnosis):

    def __init__(self, df: pd.DataFrame, columns: list|type(None) = None):
        '''
        Initializing the Categorical Diagnosis

        Parameters:
        -----------
            df: pd.DataFrame
                A pandas DataFrame

            columns : list or type(None)
                A list column names. 

                Use column names when you want to apply diagnosis only on the desired columns

        ''' 
        super().__init__(df, columns)

        self.df = df.select_dtypes(include=['object', 'string', 'category'])

        if columns is not None:
            self.columns = [column for column in df.columns if column in self.df.columns]
        else:
            self.columns = df.columns

    def get_categorical_columns(self)-> pd.DataFrame:
        '''
        Separates categorical columns from rest of the DataFrame

        Parameters:
            self

        Return:
            pd.DataFrame
            A pandas DataFrame of categorical columns
        
        Usage Recommendation:
        ---------------------
            Use this function when you want to separate categorical data from rest of the data (including other text data)

        Example: 
            CategoricalDiagnosis(df).get_categorical_columns()
            
            '''
        df = self.df.select_dtypes(include=['object', 'string', 'category'])

        # length of data
        n = len(df)
        # keeping a separate list for categorical columns
        categorical_columns = []

        for column in df.columns:
            # using a threshold of 5% unique values for considering a column as categorical
            if (len(df[column].unique())) < n * 0.05:
                categorical_columns.append(column)

        return df[categorical_columns]
        
    def show_unique_categories(self):
        
        unique_categories = {}

        for column in self.df[self.columns]:
            unique_categories[column]=len(self.df[column].unique())

        return unique_categories

    def show_frequency(self, method='sum'):
        '''
        Shows frequency count ('sum' or 'percent') for each column of Categorical DataFrame

        Parameters:
        -----------

        Optional:

            method : str (default is 'sum')

                Available methods:

                - 'sum': Calculates the sum of values in each category (default)
                - 'percent': Calculates the percentage of values in each category

                    
        '''
        # using a dictionary for storing frequency of category values
        frequency_count = {}

        # getting the percentage
        # renaming as frequency count and rounding off to 2 places 
        for column in self.df[self.columns]:
            
            if method == 'sum':
                frequency_count[column]=self.df[column].value_counts().rename(f'Frequency Count')

            elif method == 'percent':
                frequency_count[column]=self.df[column].value_counts(normalize=True).rename(f'Frequency Percentage').round(2)

            else:
                raise ValueError("Unknown method. Valid methods: 'sum' (default) or 'percent'")
        # returning the dictionary of frequency percentage
        
        return frequency_count
    
    